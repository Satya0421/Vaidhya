...
const verifyLogin = (req, res, next) => { // to verify the session is valid or not
  if (req.session.loggedIn) {
    ...
    res.redirect("/adminPanel/Vaidhya/login",)
}
//// *************** Account***************************///
      ...
      if (user) {
        ...
        await bcrypt.compare(req.body.password, user.password).then((status) => {
          if (status) {
            ...
            res.redirect('/adminPanel/Vaidhya/') // redirect used to access same page which is accessed before
          }
          else {
  ...
  if (req.files != null) {
    if (req.files.Image) {
      ...
      await db.get()
        ...
        .then((result) => {
          res.redirect("/adminPanel/Vaidhya/");
        })
...
//     ).then(async(value)=>{
//       res.redirect('/adminPanel/Vaidhya/view-inactiveDoctors');
//     });
//   } catch (error) {
